...
$a=audit_on_update($admin); $s=$conn->prepare("UPDATE customer SET is_deleted=1,date_edited=:date_edited,edited_by=:edited_by WHERE customer_id=:id"); $s->execute([':date_edited'=>$a['date_edited'],':edited_by'=>$a['edited_by'],':id'=>$id]);
audit_log('customer',$id,'DELETE',$oldData,['is_deleted'=>1],$admin); $_SESSION['alert']=['type'=>'success','message'=>'Customer deleted']; header('Location: ../../pages/customer.php'); exit;
