...
$a=audit_on_update($admin); $s=$conn->prepare("UPDATE truck SET is_deleted=1,date_edited=:date_edited,edited_by=:edited_by WHERE truck_id=:id"); $s->execute([':date_edited'=>$a['date_edited'],':edited_by'=>$a['edited_by'],':id'=>$id]);
audit_log('truck',$id,'DELETE',$oldData,['is_deleted'=>1],$admin); $_SESSION['alert']=['type'=>'success','message'=>'Truck deleted']; header('Location: ../../pages/truck.php'); exit;
